...
<nav class="navbar navbar-inverse" role="navigation" style="border-radius: 4px;">
  <div class="container-fluid">
    ...
    <ul class="nav navbar-nav navbar-right">
    	<li>
    		<h3 style="color:yellow;margin: 20px;"><span class="glyphicon glyphicon-user"> </span> <?php echo $_SESSION['id']; ?></h3>
    	</li>
			<li>
				<a href="./logout.php?id=<?php echo $_SESSION['id']?>"><button type="button" class="btn btn-primary"><span class="glyphicon glyphicon-off"></span>  LogOut</button></a>
			</li>
      <li>
          <a href="./managerSignup.php"><button type="button" class="btn btn-warning"><span class="glyphicon glyphicon-user"></span><span class="glyphicon glyphicon-user"></span><b> ADD Manager</b></button></a>
      </li>
      <li>
          <a href="./resetpwd.php"><button type="button" class="btn btn-danger"><b>Reset Password</b></button></a>
      </li>
    </ul>
  </div>
</nav>
...
<div class="container">
  <?php
    ...
    //$i = 1; 
    $sql = "SELECT *FROM managerrepo WHERE is_approved='1' AND status='0' ";
    ...
    <tbody>
      <tr>
        ...
        <td><input type="radio" onchange="hold(this.value);" value ="<?php echo $_SESSION['abc'];?>"/></td>
      </tr>
    </tbody>
  </table>
  <?php
...
<div class="container">
  <?php 
     if (isset($_POST['submit'])) {
              ...
              include "connection.php";              
              $sql = "SELECT *FROM managerrepo WHERE dayte BETWEEN '$from' AND '$to' AND status='2' ";
              $result = $conn->query($sql);
              if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
...
<div class="container">
  <?php 
    ...
    include "connection.php";
    //$i = 1; 
    $sql = "SELECT *FROM managerrepo WHERE is_approved='1' AND status='5' ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
...
<div class="container">
  <?php
    ...
    include "connection.php";
    //$i = 1; 
    $sql = "SELECT *FROM managerrepo1 WHERE is_approved='1' AND status='0' ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
    ...
    <tbody>
      <tr>
        ...
        <td><input type="radio" onchange="hold1(this.value);" value ="<?php echo $_SESSION['abc'];?>"/></td>
      </tr>
    </tbody>
  </table>
  <?php
    ...
    <h2></h2>
       <form class="form-horizontal" role="form" action="<?php echo $_SERVER['PHP_SELF']; ?>" method="POST">
          <div class="form-group">
              ...
              <label for="to" class="col-sm-1 control-label">To :</label>
              <div class="col-sm-2">
                  <input type="text"  name="to" id="dp4" class="form-control"  placeholder="Date like: 2017-02-23">
              </div>
                  <div class="col-sm-2 col-sm-offset-1">
                    <button type="submit" name="submit" class="btn btn-primary btn-block">Submit</button>
                  </div>
...
<div class="container">
  <?php 
   if (isset($_POST['submit'])) {
              ...
              include "connection.php";              
              $sql = "SELECT *FROM managerrepo1 WHERE dayte BETWEEN '$from' AND '$to' AND status='2' ";
              $result = $conn->query($sql);
              if ($result->num_rows > 0) {
                // output data of each row
                while($row = $result->fetch_assoc()) {
...
<div class="container">
  <?php 
    ...
    include "connection.php";
    //$i = 1; 
    $sql = "SELECT *FROM managerrepo1 WHERE is_approved='1' AND status='5' ";
    $result = $conn->query($sql);
    if ($result->num_rows > 0) {
      // output data of each row
      while($row = $result->fetch_assoc()) {
